...
# Pages Functions（/api/*）。整个目录复制，其中不应存在任何机密：
# DATABASE_URL 等以 Cloudflare 的 secret 形式注入，不进代码库、也不进这个包。
cp -R "$ROOT/functions" "$OUT/functions"

# 数据隔离：dev 与 prod 共用一个 Neon 库，靠这个常量分桶写入。
# 由部署脚本生成而非仪表板配置，是为了让「这次部署写进哪个桶」在版本里可追溯。
printf '// generated by deploy.sh — do not edit\nexport const APP_ENV = %s;\n' \
  "$([ "$BRANCH" = "main" ] && echo "'prod'" || echo "'dev'")" > "$OUT/functions/_env.js"
echo "  ✓ APP_ENV=$([ "$BRANCH" = "main" ] && echo prod || echo dev)"
