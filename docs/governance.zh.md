...
| 档 | 要求 | 说明 |
|---|---|---|
| **线索** | 1 个链接 | 「我看到一条新闻，你们看看」。不进档案，只进待办队列 |
| **补来源** | 1 个链接 | 给已有条目补上可点击来源 |
| **纠错** | 1 个链接 + 说明 | 指出错在哪、依据是什么 |
| **完整新增** | ≥2 个独立来源 + 说明 | 满足上述四项准则，才可能成为档案条目 |

分档的理由：如果「丢个线索」和「新增一条条目」要填一样多的字，两种人都不会填。
**四档全部只进复核队列，没有任何一档能直接上线。**

标题与日期由系统从链接自动读取，投稿者写的是「发生了什么、改变了什么」——
那是机器抄不来的部分。**不要整段复制原文**：事实不受著作权保护，表达受保护；
档案记事实、附链接，不搬段落。
...
不要求任何人相信运营者。以下是本平台承诺的保证。

> **当前阶段：架构期。** 其中多数已设计但尚未运行，逐条标注真实状态。
> 我们宁可公布一份诚实的路线图，也不宣称并不存在的能力——
> 一个夸大自己做到什么的平台，已经先违背了它要求别人遵守的标准。

| # | 保证 | 状态 |
|---|---|---|
| 1 | **每日公开导出** —— 完整数据集（`export_events` 视图）导出为 JSON 推送至公开仓库，任何人可整包带走、自行分析、甚至另建一个。**退出成本为零。** | ⬜ 计划中 —— 需要数据库与导出任务 |
| 2 | **修订快照** —— `revisions` 表记录每次变更状态；页面上任何一句话都能查到是谁、何时、依据什么改的。 | 🟨 结构已定义（见 [`db/schema.sql`](../db/schema.sql)，含触发器），尚无数据流经 |
| 3 | **中立性自检** —— `event_balance` 视图与 `check_publishable()` 函数，任何人可自行验证是否真的正反并列。 | 🟨 已写好且公开可读；有数据后即可运行 |
| 4 | **纠错台账** —— `corrections` 表公开，包括**尚未解决**的请求。一个只展示"已修正"的平台是在挑好看的说。 | ⬜ 计划中 —— 需要提交入口 |
| 5 | **开源** —— 前端、数据结构与本准则全部公开，可逐行审阅。 | ✅ 现已生效 |
